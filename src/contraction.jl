# TCI-specific contraction code
# The basic contract, contract_naive, contract_zipup are now in T4ATensorTrain.jl
...
- `algorithm=:TCI`: TCI-based contraction (implemented in this package).
- `algorithm=:naive|:zipup`: delegated to `T4ATensorTrain.contract` (non-TCI).
"""
function contract(
    A::TensorTrain{V1,4},
    B::TensorTrain{V2,4};
    ...
    f === nothing || error("Only algorithm=:TCI supports `f`. Use algorithm=:TCI or set f=nothing.")
    return T4ATensorTrain.contract(A, B; algorithm=algorithm, tolerance=tolerance, maxbonddim=maxbonddim, kwargs...)
end

# Re-export contract_naive with Contraction argument
function contract_naive(
    ...
    a, b = obj.mpo
    tt = TensorTrain{T,4}(T4ATensorTrain._contractsitetensors.(sitetensors(a), sitetensors(b)))
