"""
    T4AITensorCompat.TensorTrain(tt::TCI.TensorTrain{ValueType,N}; sites=nothing)

Convert a `T4ATensorCI.TensorTrain` to a `T4AITensorCompat.TensorTrain`.

# Arguments
- `tt::TCI.TensorTrain{ValueType,N}`: The tensor train to convert
...
"""
function T4AITensorCompat.TensorTrain(tt::TCI.TensorTrain{ValueType,N}; sites=nothing) where {ValueType,N}
    N_sites = length(tt)
...
"""
    T4AITensorCompat.TensorTrain(tt::TCI.AbstractTensorTrain{ValueType}; sites=nothing)

Convert an `AbstractTensorTrain` to a `T4AITensorCompat.TensorTrain` by first converting to `TensorTrain`.

# Arguments
- `tt::TCI.AbstractTensorTrain{ValueType}`: The tensor train to convert
- `sites`: Optional site indices. If `nothing`, default indices will be generated.

# Returns
- `T4AITensorCompat.TensorTrain`: The converted tensor train
"""
function T4AITensorCompat.TensorTrain(tt::TCI.AbstractTensorTrain{ValueType}; sites=nothing) where {ValueType}
    return T4AITensorCompat.TensorTrain(TCI.tensortrain(tt); sites=sites)
end

"""
    TCI.TensorTrain(tt::T4AITensorCompat.TensorTrain; sites=nothing)
    TCI.TensorTrain{V, N}(tt::T4AITensorCompat.TensorTrain; sites=nothing)

Convert a `T4AITensorCompat.TensorTrain` to a `T4ATensorCI.TensorTrain`.
...
# Returns
- `TCI.TensorTrain{ValueType,N}`: The converted tensor train
"""
function TCI.TensorTrain(tt::T4AITensorCompat.TensorTrain; sites=nothing)
    N = length(tt)
    N == 0 && return TCI.TensorTrain(Vector{Array{Float64,3}}())
    ...
    if is_mps
        ...
        error("MPO conversion from T4AITensorCompat.TensorTrain to T4ATensorCI.TensorTrain is not yet implemented. Please convert via ITensorMPS.MPO first.")
    end
end

function TCI.TensorTrain{V, N}(tt::T4AITensorCompat.TensorTrain; sites=nothing) where {V, N}
    N_tt = length(tt)
    N_tt == 0 && return TCI.TensorTrain{V, N}(Vector{Array{V, N}}())
